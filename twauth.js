...
const createAuthHeader = (requestMethod, requestedUrl, params) => {
    let authParams = {
        oauth_consumer_key: credentials.consumer.key,
        oauth_token: credentials.dummyUser.accessToken,
    ...
    let accessTokenSecret = credentials.dummyUser.secret
...
const createRequestOptions = (requestMethod, path, params) => {
    let uriPath = path
    if(params) {
        let paramsStr = qs.stringify(params)
        uriPath = `${path}?${paramsStr}`
    }
    let authHeader = createAuthHeader(requestMethod, `https://api.twitter.com${path}`, params)
...
exports.request = (method, path, params) => new Promise((resolve, reject) => {
    let paramsStr = params ? qs.stringify(params) : undefined
    let options = createRequestOptions(method, path, params)
